...
function merge(a,b){const level=Math.min(a.l+1,fruits.length-1);a.dead=b.dead=true;const f=fruits[level];balls.push({x:(a.x+b.x)/2,y:(a.y+b.y)/2,vx:(a.vx+b.vx)/2,vy:-80,l:level,r:f.r,merge:.18});score+=f.s;pop(a.x,a.y,`+${f.s}`);updateHud()}
...
function step(dt){if(state!=='play')return;for(const b of balls){b.vy+=900*dt;b.x+=b.vx*dt;b.y+=b.vy*dt;b.vx*=.995;if(b.x-b.r<34){b.x=34+b.r;b.vx=Math.abs(b.vx)*.55}if(b.x+b.r>W-34){b.x=W-34-b.r;b.vx=-Math.abs(b.vx)*.55}if(b.y+b.r>H-34){b.y=H-34-b.r;b.vy=-Math.abs(b.vy)*.35;b.vx*=.86}if(b.merge>0)b.merge-=dt}
 for(let k=0;k<3;k++)for(let i=0;i<balls.length;i++)for(let j=i+1;j<balls.length;j++){const a=balls[i],b=balls[j];if(a.dead||b.dead)continue;let dx=b.x-a.x,dy=b.y-a.y,d=Math.hypot(dx,dy)||1,min=a.r+b.r;if(d<min){let push=(min-d)/2,nx=dx/d,ny=dy/d;a.x-=nx*push;b.x+=nx*push;a.y-=ny*push;b.y+=ny*push;let rv=(b.vx-a.vx)*nx+(b.vy-a.vy)*ny;if(rv<0){let imp=-rv*.45;a.vx-=nx*imp;b.vx+=nx*imp;a.vy-=ny*imp;b.vy+=ny*imp}if(a.l===b.l&&a.merge<=0&&b.merge<=0&&d<min*.72){merge(a,b)}}}
